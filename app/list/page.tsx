...
type SearchParamsInput = SearchParams | URLSearchParams
...
function getParam(params: SearchParamsInput | undefined, key: string) {
  if (!params) return ""
    ...
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-40 bg-background/95 backdrop-blur-sm border-b border-border">
        <div className="px-4 py-4 space-y-1">
          <h1 className="text-xl font-semibold text-foreground">Список студентов</h1>
      ...
      <main className="px-4 py-6 space-y-6">
        ...
        {totalFiltered > 0 && totalPages > 1 && (
          <nav className="flex items-center justify-between gap-3 text-sm">
            <a
              href={`/list${buildQueryString(paginationFilters, { page: prevPage })}`}
            ...
            <a
              href={`/list${buildQueryString(paginationFilters, { page: nextPage })}`}
              className={`inline-flex items-center justify-center rounded-xl border border-border px-4 py-2 transition ${
                page >= totalPages
                  ? "pointer-events-none text-muted-foreground"
                  : "hover:border-foreground/20"
